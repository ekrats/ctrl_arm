...
static rt_err_t AT45DB_flash_control(rt_device_t dev, rt_uint8_t cmd, void *args)
{
    ...
    {
        struct ty_at45_erase_set * erase_info;
        uint8_t send_buffer[4];
        erase_info = (struct ty_at45_erase_set *)args;
        switch(erase_info->type)
        {
            case AT45_ERASE_PAGE :
                send_buffer[0] = AT45DB_PAGE_ERASE;
                send_buffer[1] = (uint8_t)(erase_info->erase_para >> 6);
                send_buffer[2] = (uint8_t)(erase_info->erase_para << 2);
                send_buffer[3] = 0;
                rt_spi_send(spi_flash_at45dbxx.rt_spi_device, send_buffer, 4);
                
                break;
            case AT45_ERASE_CHIP :
                send_buffer[0] = 0xC7;
                send_buffer[1] = 0x94;
                send_buffer[2] = 0x80;
                send_buffer[3] = 0x9A;
